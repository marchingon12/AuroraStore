...
class SelfUpdateWorker @AssistedInject constructor(
        ...
        try {
            ...
            if (selfUpdate.versionCode > BuildConfig.VERSION_CODE) {
                if (CertUtil.isFDroidApp(appContext, BuildConfig.APPLICATION_ID)) {
                    if (selfUpdate.fdroidBuild.isNotEmpty()) saveUpdateFile(selfUpdate)
                } else if (selfUpdate.auroraBuild.isNotEmpty()) {
                    saveUpdateFile(selfUpdate)
                } else {
    ...
    private fun saveUpdateFile(selfUpdate: SelfUpdate) {
        PathUtil.getUpdateFile(appContext).apply {
            createNewFile()
            outputStream().use { writeText(gson.toJson(selfUpdate)) }
        }
    }
}
